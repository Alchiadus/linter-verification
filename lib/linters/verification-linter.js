...
export default class VerificationLinter {
  ...
  createVerificationMessages(output) {
    ...
    const warningRegExp = /(.+)\((\d+),(\d+)\): [Ww]arning(?:| [\w\d]+): (.+)/;
    ...
    stdoutLines.forEach((line) => {
      ...
      const verificationWarning = line.match(warningRegExp);
      if (verificationWarning !== null) {
        const type = 'Warning';
        const lineNumber = verificationWarning[2];
        const column = verificationWarning[3];
        const text = verificationWarning[4];
        messages.push({
          type,
          text,
          range: [
    ...
    const verificationFinished = lastLine.match(finishedRegExp);
    let type;
    if (this.trustExitCode()) {
      type = output.exitCode === 0 ? 'Success' : 'Info';
    } else if (verificationFinished !== null) {
      const unsuccessfulVerifications = verificationFinished[2];
      type = unsuccessfulVerifications === '0' ? 'Success' : 'Info';
    } else {
      type = 'Info';
    }
    messages.push({
